...
const terminalLines = [
  { prompt: '$', cmd: 'git log --oneline -3', out: false },
  { prompt: null, cmd: 'a3f91c2 feat: add sensor fusion layer', out: true },
  { prompt: null, cmd: 'b82e014 fix: uart buffer overflow on ESP32', out: true },
  { prompt: null, cmd: '4d1cc87 refactor: decouple MQTT from core', out: true },
  { prompt: '$', cmd: 'make flash && make monitor', out: false },
  { prompt: null, cmd: '✓ build succeeded · 182KB / 4MB flash', out: true },
]
  ...
  return (
    <section className="hero">
      <div className="hero-content">
        <h1>
          <span className="glow">Code</span>, <span className="italic">circuits</span><br />
          & clean<br />
          <span className="glow">abstractions</span>.<span className="cursor"></span>
        </h1>
        <p className="hero-subtitle">
          ...
          <span className="key">build</span>: <span className="str">passing</span> · <span className="key">ci</span>: <span className="num">✓</span><br />
      ...
      <div className="hero-right">
        ...
        <div className="terminal">
          <div className="terminal-header">
            ...
            <span className="terminal-title">tristan@dev ~ </span>
          </div>
          <div className="terminal-body">
            {terminalLines.map((line, i) => (
              <div key={i} className={line.out ? 'terminal-out' : 'terminal-cmd'}>
                {!line.out && <span className="terminal-prompt">$</span>}
                <span>{line.cmd}</span>
              </div>
            ))}
